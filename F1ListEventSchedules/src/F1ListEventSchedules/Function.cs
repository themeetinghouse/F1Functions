...
namespace F1ListEventSchedules;
...
public class Function
{
    ...
    public object? FunctionHandler(InputTest input, ILambdaContext context)
    {
        ...
        f1OAuthTicket = RestClient.Authorize(f1OAuthTicket,str,str1, LoginType.PortalUser, false);
        RestClient restClient = new RestClient(f1OAuthTicket, false, false);
        EventRealm eventRealm = new EventRealm(f1OAuthTicket, "https://meetinghouse.fellowshiponeapi.com/", restClient.ContentType);
        ConcurrentBag<string> concurrentBag = new ConcurrentBag<string>();
        Parallel.ForEach<string>(input.arguments.itemId, new Action<string>( (string str) => {
            string jsonResponse = eventRealm.Events.Get(str).JsonResponse;
            concurrentBag.Add(string.Concat(new string[] { "{\"id\":\"", str, "\",\"event\":", jsonResponse, "}" }));
        }));
        string strA = "[";
